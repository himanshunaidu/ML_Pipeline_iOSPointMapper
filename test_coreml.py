...
from utilities.main_file_utils import get_dataset_config, get_default_weights_test, \
...
def get_save_dir(args: TestConfig) -> str:
    ...
    if args.dataset == 'city':
        savedir = 'results_test_coreml/{}_{}_{}'.format('results', args.dataset, args.split)
        ...
        savedir = 'results_test_coreml/{}_{}/VOC2012/Segmentation/comp6_{}_cls'.format('results', args.dataset, args.split)
...
def preprocess_inputs(output, target, is_output_probabilities=False):
...
def test(args, model, dataset_loader: torch.utils.data.DataLoader, device):
    im_size = tuple(args.im_size)
    args.cmap = get_cmap(args)   

    custom_eval = CustomEvaluation(num_classes=args.classes, max_regions=1024, is_output_probabilities=False, 
    ...
    for index, (inputs, target) in tqdm(enumerate(dataset_loader), total=len(dataset_loader)):
        ...
        for i in range(img_out.shape[0]):
            ...
            input_i_save = inputs[i].squeeze(0)
...
def main(args: TestConfig):
    ...
    # Set mean and std to default values since the coreml model already has them
    ...
    test(args, model, dataset_loader, device='cpu')
    recolor_images(args)
    evaluate(args)
...
if __name__ == '__main__':
    ...
    # general details
    parser.add_argument('--config', type=str, default='config/coreml/test_segmentation_bisenetv2_coreml.json', help='Path to the config file')
