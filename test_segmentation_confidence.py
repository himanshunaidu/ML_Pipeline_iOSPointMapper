"""
This script is used to test the segmentation models on various datasets for their confidence scores.
Currently, only BiSeNetv2 is actually supported.
"""
import torch
import glob
import os
...
def evaluate(args, model, image_list, device):
        ...
        if args.dataset == 'edge_mapping' and args.is_custom:
            ...
            for k, v in CITYSCAPE_TRAIN_CMAP.items():
                if k in edge_mapping_to_custom_cocoStuff_dict.keys():
                    ...
                    cmap[k] = v
    elif args.dataset == 'hand_labelled':
        from data_loader.semantic_segmentation.hand_labelled import HandLabelledSegmentation
        dataset = HandLabelledSegmentation(root=args.data_path, split=args.split, size=args.im_size, scale=args.s,
                                           is_custom=args.is_custom,
                                           custom_mapping_dict=args.custom_mapping_dict)
        cmap = dataset.cmap
        # print(cmap[41], cmap[35], cmap[19], cmap[50], cmap[24], cmap[0], cmap[8], cmap[11], cmap[31], cmap[27], cmap[0])
        # exit(-1)
    else:
        cmap = None
    ...
    for index, imgName in tqdm(enumerate(image_list)):
        ...
        img_out = softmax(img_out, dim=0)  # apply softmax to get probabilities
        img_out = img_out.cpu().detach().numpy()  # convert to numpy array
        img_out = np.max(img_out, axis=0)  # get the 'confidence' map
        # img_out = img_out.max(0)[1].byte()  # get the label map
        # img_out = img_out.to(device='cpu').numpy()

        # Normalize the confidence map to [0, 255]
...
def main(args):
    ...
    #     seg_classes = len(EDGE_MAPPING_CLASS_LIST)
        ...
        seg_classes = len(dataset.labels)
        if args.is_custom: seg_classes = 53
        print_info_message('Using {} classes for segmentation'.format(seg_classes))
    else:
    ...
    if args.model == 'espnetv2':
        ...
        elif args.dataset == 'hand_labelled' and args.is_custom:
            args.mean = (0.46962251, 0.4464104,  0.40718787)
            args.std = (0.27469736, 0.27012361, 0.28515933)
        else:
            args.mean = MEAN
...
if __name__ == '__main__':
    ...
    # set-up results path
    if args.dataset == 'city':
        args.savedir = 'results_confidence/{}_{}_{}'.format('results', args.dataset, args.split)
    elif args.dataset == 'edge_mapping': # MARK: edge mapping dataset
        args.savedir = 'results_confidence/{}_{}/{}'.format('results', args.dataset, args.split)
    elif args.dataset == 'pascal':
        args.savedir = 'results_confidence/{}_{}/VOC2012/Segmentation/comp6_{}_cls'.format('results', args.dataset, args.split)
    elif args.dataset == 'hand_labelled':
        args.savedir = 'results_confidence/{}_{}/{}'.format('results', args.dataset, args.split)
    else:
        print_error_message('{} dataset not yet supported'.format(args.dataset))
