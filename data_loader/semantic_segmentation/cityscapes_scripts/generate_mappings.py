"""
Creates pairs of image and annotation file paths and saves them in a text file.
"""
import glob
import os
import sys
...
def main(cityscapesPath, split):
    ...
    if split == 'train':
        ...
        filesCoarse.sort()
        if not filesCoarse:
            print_warning_message("Did not find any coarse files. Please check root directory: {}.".format(cityscapesPath))
            course_pairs = []
        else:
